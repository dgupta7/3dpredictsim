...
function [r1,r2,r3,r4,r5] = f_Foot(Qtib_rz,Qtib_ty,Qankle,Qsubt,Qtmt,Qmtp,F_tib,F,PassT_Foot,jointi)
    import casadi.*
    nqal = 10; % n dof

    % passive torques
    Qs_pass = [Qankle,Qsubt,Qtmt,Qmtp]; 
    Tau_pass = PassT_Foot(Qs_pass);

    % dynamics
    qs = SX.sym('qs',nqal);
    qs(:) = 0;
    qs(jointi.tibia.rz) = Qtib_rz;
    qs(jointi.tibia.ty) = Qtib_ty;
    qs(jointi.ankle.r) = Qankle;
    qs(jointi.subt.r) = Qsubt;
    qs(jointi.tmt.r) = Qtmt;
    qs(jointi.mtp.r) = Qmtp;
    A = zeros(nqal,1);
    QsQdots_nsc = SX.sym('QsQdots',nqal*2);
    QsQdots_nsc(:) = 0;
    QsQdots_nsc(1:2:end,:) = qs;

    [Tj] = F([QsQdots_nsc(:,1);A(:,1);0]);

    % positions
    % toes_or_sx = Tj(jointi.toes_or);
    metatarsi_or_sx = Tj(jointi.metatarsi_or);
    calcn_or_sx = Tj(jointi.calcn_or);
    % talus_or_sx = Tj(jointi.talus_or);
    tibia_or_sx = Tj(jointi.tibia_or);

    % make function
    r1 = (metatarsi_or_sx(2)+calcn_or_sx(2))/2 - tibia_or_sx(2); % position of knee
    r2 = Tj(jointi.tibia.ty,1)+ F_tib; % vertical force on knee
    r3 = Tj(jointi.ankle.r,1) - Tau_pass(1); % ankle
    r4 = Tj(jointi.subt.r,1) - Tau_pass(2); % subt
    r5 = Tj(jointi.tmt.r,1) - Tau_pass(3); % tmt
...
F_tib_y = SX.sym('F_tib_y ',1);

[f1,f2,f3,f4,f5] = f_Foot(Q_tib_rz,Q_tib_ty,Q_ankle,Q_subt,Q_tmt,Q_mtp,F_tib_y,F,f_getPassT_Foot,jointfi);
