function [] = f_PredSim_StiffnessAFO(S)
...
% stiffness of AFO
if ~isfield(S,'AFO_stiffness') || isempty(S.AFO_stiffness)
...
if solveProblem
    ...
    for j=1:d
        ...
        GRF_r = Tj(GRFi.r); GRFrY = GRF_r(2);        
        GRF_l = Tj(GRFi.l); GRFlY = GRF_l(2);  
        ...
        eq_constr{end+1} = Tj(jointi.ankle.l,1)-(T_ankle_l + Tau_passj.ankle.l  + TAFO_l);
        % Ankle, right
        Ft_ankle_r      = FTj(mai(5).mus.r',1);
        T_ankle_r       = f_T12(MAj.ankle.r,Ft_ankle_r);
        eq_constr{end+1} = Tj(jointi.ankle.r,1)-(T_ankle_r + Tau_passj.ankle.r + TAFO_r);
        % Subtalar, left
