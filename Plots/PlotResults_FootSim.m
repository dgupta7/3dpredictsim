...
tab1 = uitab(hTabGroup, 'Title', 'Windlass');
tab2 = uitab(hTabGroup, 'Title', 'Positions');
tab3 = uitab(hTabGroup, 'Title', 'Arch');
...
for i=1:n_mtp
    ...
    subplot(3,3,2)
    hold on
    pl1=plot(Fs_tib,R.h_fa(i,js)*1000,'--','Color',CsV(i,:),'DisplayName','Windlass model');
    pl2=plot(Fs_tib,R.h_fa_ext(i,js)*1000,'Color',CsV(i,:),'DisplayName','External func');
    xlabel('tibia force (N)')
    ylabel('arch height (mm)')
    title('Foot arch height (sagittal)')
    lg0=legend([pl1,pl2],'Location','northeast');

%     subplot(3,3,3)
%     hold on
%     plot(Fs_tib,R.Qs(i,:,R.jointfi.tmt.r)*180/pi,'Color',CsV(i,:))
%     xlabel('tibia force (N)')
%     ylabel('tmt angle (°)')
...
axes('parent', tab3);
subplot(1,3,1)
plot((R.l_fa(j,js)- R.L0)*1000,Fs_tib/1000)
hold on
...
subplot(1,3,2)
plot((R.H0-R.h_fa(j,js))*1000,Fs_tib/1000)
hold on
