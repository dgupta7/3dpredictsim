function [R] = f_LoadSim_Gait92_mtj_PIM(ResultsFolder,loadname)
...
a_PIM_opt = reshape(w_opt(starti:starti+nq.PIM*(N+1)-1),nq.PIM,N+1)';
starti = starti + nq.PIM*(N+1);
a_PIM_col_opt = reshape(w_opt(starti:starti+nq.PIM*(d*N)-1),nq.PIM,d*N)';
starti = starti + nq.PIM*(d*N);
...
e_PIM_opt = reshape(w_opt(starti:starti+nq.PIM*N-1),nq.PIM,N)';
starti = starti + nq.PIM*N;
...
a_PIM_mesh_col_opt=zeros(N*(d+1)+1,nq.PIM);
...
for k=1:N
    for j=1:d
        ...
        v_PIM_l = f_getPFvelocity(q_col_opt_unsc.rad(count,jointi.tmt.l),...
            qdot_col_opt_unsc.rad(count,jointi.tmt.l),q_col_opt_unsc.rad(count,jointi.mtp.l),...
            qdot_col_opt_unsc.rad(count,jointi.mtp.l));
        v_PIM_r = f_getPFvelocity(q_col_opt_unsc.rad(count,jointi.tmt.r),...
            qdot_col_opt_unsc.rad(count,jointi.tmt.r),q_col_opt_unsc.rad(count,jointi.mtp.r),...
            qdot_col_opt_unsc.rad(count,jointi.mtp.r));
        P_PIM_2 = -[v_PIM_l,v_PIM_r].*F_PIM_col;
        ...
        J_opt = J_opt + 1/(dist_trav_opt)*(...
            ...
            W.PIM*B(j+1) * (f_J2(e_PIM_opt(k,:)))*h_opt +...
            W.Ak*B(j+1) * (f_J25(qdotdot_col_opt(count,residuals_noarmsi)))*h_opt +...
...
Tau_pass_opt_inv = [jointi.hip_flex.r:jointi.hip_rot.r,...
    ...
    jointi.subt.r,jointi.subt.l,jointi.tmt.r,jointi.tmt.l,jointi.mtp.r,jointi.mtp.l,...
...
if S.mtj
    ...
    l_PF = zeros(length(Qs_GC),1);
    MA_PF = zeros(length(Qs_GC),1);
    M_mtp = zeros(length(Qs_GC),1);
...
%         F_PF(i) = F_PFi;
        ...
        [~,M_mtpi, M_PFi,M_lii,~,F_PFi,l_PFi,MA_PFi,~,h_fai,l_fai] = ...
                ...
                Qs_GC(i,jointi.mtp.r)*pi/180,f_PF_stiffness,S,F_PIM(i));
...
R.colheaders.dM = {'hip flex','hip add','hip rot','knee angle','ankle angle',...
    'subtalar angle','mtp angle', 'trunk ext ','trunk bend','trunk rot'};
