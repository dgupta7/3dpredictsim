function [] = f_PredSim_Gait92_foot_muscles(S)
...
nq.leg      = 11; % #joints needed for polynomials
nq.PIM      = 2; % Plantar Intrinsic Muscles, 1 set for each foot
...
tl = load([pathpolynomial,'/muscle_spanning_joint_INFO_subject1_mtj.mat']);
...
f_passiveWLTorques_mtj_PIM = Function.load(fullfile(PathDefaultFunc,'f_passiveWLTorques_mtj_PIM'));
f_passiveWLTorques_mtp_PIM = Function.load(fullfile(PathDefaultFunc,'f_passiveWLTorques_mtp_PIM'));
f_getPFvelocity = Function.load(fullfile(PathDefaultFunc,'f_getPFvelocity'));
...
elseif S.IGsel == 2 % Data-informed initial guess
    ...
    elseif S.IGmodeID == 3 || S.IGmodeID == 4 % Data from selected motion
        ...
        frSel = round(nfr./4);
        Qs_ig.allfilt(:,6) = Qs_ig.allfilt(:,6);
...
a_PIMk      = MX.sym('a_PIMk',nq.PIM);
a_PIMj      = MX.sym('a_PIMkmesh',nq.PIM,d);
...
for j=1:d
    ...
    MAj.mtj.l       =   MAj_l(mai(7).mus.l',7);
    MAj.mtp.l       =   MAj_l(mai(8).mus.l',8);
    ...
    MAj.trunk_ext    =  MAj_l([47:49,mai(9).mus.l]',9);
    MAj.trunk_ben    =  MAj_l([47:49,mai(10).mus.l]',10);
    MAj.trunk_rot    =  MAj_l([47:49,mai(11).mus.l]',11);
    % Right leg
    ...
    MAj.mtj.r       =   MAj_r(mai(7).mus.l',7);
    MAj.mtp.r       =   MAj_r(mai(8).mus.l',8);
    % Both legs
    ...
    F_PIM.l = a_PIMkj(1,j+1)*scaling.PIMF;
    F_PIM.r = a_PIMkj(2,j+1)*scaling.PIMF;

    Tau_passj.mtj.l = f_passiveWLTorques_mtj_PIM(Qskj_nsc(jointi.mtj.l,j+1),...
        Qdotskj_nsc(jointi.mtj.l,j+1),Qskj_nsc(jointi.mtp.l,j+1),F_PIM.l);
    Tau_passj.mtj.r = f_passiveWLTorques_mtj_PIM(Qskj_nsc(jointi.mtj.r,j+1),...
        Qdotskj_nsc(jointi.mtj.r,j+1),Qskj_nsc(jointi.mtp.r,j+1),F_PIM.r);

    Tau_passj.mtp.l = Tau_passj.mtp.all(1) +...
        f_passiveWLTorques_mtp_PIM(Qskj_nsc(jointi.mtj.l,j+1),...
        Qdotskj_nsc(jointi.mtj.l,j+1),Qskj_nsc(jointi.mtp.l,j+1),F_PIM.l);
    Tau_passj.mtp.r = Tau_passj.mtp.all(2) +...
        f_passiveWLTorques_mtp_PIM(Qskj_nsc(jointi.mtj.r,j+1),...
        Qdotskj_nsc(jointi.mtj.r,j+1),Qskj_nsc(jointi.mtp.r,j+1),F_PIM.r);
    ...
    eq_constr{end+1} = Tj(jointi.mtj.l,1)-(T_mtj_l + Tau_passj.mtj.l);
    % mtj right
    Ft_mtj_r        = FTj(mai(7).mus.r',1);
    T_mtj_r         = f_T9(MAj.mtj.r,Ft_mtj_r);
    eq_constr{end+1} = Tj(jointi.mtj.r,1)-(T_mtj_r + Tau_passj.mtj.r);
    % Mtp left
    Ft_mtp_l        = FTj(mai(8).mus.l',1);
    T_mtp_l         = f_T4(MAj.mtp.l,Ft_mtp_l);
    eq_constr{end+1} = Tj(jointi.mtp.l,1)-(T_mtp_l + Tau_passj.mtp.l + a_mtpkj(1,j+1)*scaling.MtpTau);
    % Mtp right
    Ft_mtp_r        = FTj(mai(8).mus.r',1);
    T_mtp_r         = f_T4(MAj.mtp.r,Ft_mtp_r);
    eq_constr{end+1} = Tj(jointi.mtp.r,1)-(T_mtp_r + Tau_passj.mtp.r + a_mtpkj(2,j+1)*scaling.MtpTau);
    % Lumbar extension
    Ft_trunk_ext    = FTj([mai(9).mus.l,mai(9).mus.r]',1);
    T_trunk_ext     = f_T6(MAj.trunk_ext,Ft_trunk_ext);
    eq_constr{end+1} = Tj(jointi.trunk.ext,1)-(T_trunk_ext + Tau_passj.trunk.ext);
    % Lumbar bending
    Ft_trunk_ben    = FTj([mai(10).mus.l,mai(10).mus.r]',1);
    T_trunk_ben     = f_T6(MAj.trunk_ben,Ft_trunk_ben);
    eq_constr{end+1} = Tj(jointi.trunk.ben,1)-(T_trunk_ben + Tau_passj.trunk.ben);
    % Lumbar rotation
    Ft_trunk_rot    = FTj([mai(11).mus.l,mai(11).mus.r]',1);
    ...
    v_PIM_l = f_getPFvelocity(Qskj_nsc(jointi.mtj.l,j+1),Qdotskj_nsc(jointi.mtj.l,j+1),...
        Qskj_nsc(jointi.mtp.l,j+1),Qdotskj_nsc(jointi.mtp.l,j+1));
    P_PIM_l = -v_PIM_l*F_PIM.l;
    
    v_PIM_r = f_getPFvelocity(Qskj_nsc(jointi.mtj.r,j+1),Qdotskj_nsc(jointi.mtj.r,j+1),...
        Qskj_nsc(jointi.mtp.r,j+1),Qdotskj_nsc(jointi.mtp.r,j+1));
    P_PIM_r = -v_PIM_r*F_PIM.r;
    
    % extra cost function term to minimise net mechanical energy
    % generated/dissiped by both PIMs
    J_2 = J_2 + B(j+1) * (P_PIM_l+P_PIM_r)/S.mass*h;
...
f_coll = Function('f_coll',{tfk,ak,aj,FTtildek,FTtildej,Qsk,Qsj,Qdotsk,...
    Qdotsj,a_ak,a_aj,a_mtpk,a_mtpj,a_PIMk,a_PIMj,vAk,e_ak,e_mtpk,e_PIMk,dFTtildej,Aj,Texok},...
...
[coll_eq_constr, coll_ineq_constr1, coll_ineq_constr2, coll_ineq_constr3,...
    ...
    a_mtp(:,1:end-1), a_mtp_col, a_PIM(:,1:end-1), a_PIM_col, vA, e_a, e_mtp, e_PIM, dFTtilde_col, A_col,ExoVect);
% constrains
