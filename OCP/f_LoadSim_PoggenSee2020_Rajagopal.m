...
for k=1:N
    for j=1:d
        ...
        % Both legs
        lMTk_lr_opt_all = [lMTk_l_opt_all(1:40,1);lMTk_r_opt_all(1:40,1)];
        vMTk_lr_opt_all = [vMTk_l_opt_all(1:40,1);vMTk_r_opt_all(1:40,1)];
        % force equilibirum
        ...
        J_opt = J_opt + 1/(dist_trav_opt)*(...
            W.E*B(j+1) * (f_J80exp(e_tot_opt_all,exp_E))/body_mass*h_opt + ...
            W.A*B(j+1) * (f_J80(a_col_opt(count,:)))*h_opt +...
            W.ArmE*B(j+1) * (f_J8(e_a_opt(k,:)))*h_opt +...
            W.Lumbar*B(j+1)     *(f_J3(e_lumbar_opt(k,:)))*h_opt +...                   % reserve actuators lumbar
            ...
            W.u*B(j+1) * (f_J80(vA_opt(k,:)))*h_opt + ...
            W.u*B(j+1) * (f_J80(dFTtilde_col_opt(count,:)))*h_opt + ...
            W.u*B(j+1) * (f_J8(qdotdot_col_opt(count,armsi)))*h_opt);
        
        E_cost = E_cost + W.E*B(j+1)*...
            (f_J80exp(e_tot_opt_all,exp_E))/body_mass*h_opt;
        A_cost = A_cost + W.A*B(j+1)*...
            (f_J80(a_col_opt(count,:)))*h_opt;
        Arm_cost = Arm_cost + W.ArmE*B(j+1)*...
            (f_J8(e_a_opt(k,:)))*h_opt;
        Mtp_cost = Mtp_cost + W.Mtp*B(j+1)*...
            (f_J2(e_mtp_opt(k,:)))*h_opt;
        Lumbar_cost = Lumbar_cost + W.Lumbar*B(j+1)*...
            (f_J3(e_lumbar_opt(k,:)))*h_opt;
        ...
        vA_cost = vA_cost + W.u*B(j+1)*...
            (f_J80(vA_opt(k,:)))*h_opt;
        dFTtilde_cost = dFTtilde_cost + W.u*B(j+1)*...
            (f_J80(dFTtilde_col_opt(count,:)))*h_opt;
...
Lumbar_costf = full(Lumbar_cost); Obj.lumbar = Lumbar_costf;
...
assertCost = abs(J_optf - 1/(dist_trav_opt)*(E_costf+A_costf+Arm_costf+...
    Mtp_costf+Lumbar_costf+Qdotdot_costf+Pass_costf+vA_costf+dFTtilde_costf+...
    QdotdotArm_costf));
