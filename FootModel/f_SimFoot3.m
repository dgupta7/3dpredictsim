...
% opti.subject_to(bounds_qs(:,1) <= qs_opti(:,1) <= bounds_qs(:,2));
% qmtp = opti.parameter();
% Ftib = opti.parameter();
...
%         x = full(f_foot_sol(vars_init,Qs_mtp(i),Fs_tib(j)));
        x = full(f_foot(vars_init,Qs_mtp(i),Fs_tib(j)));
