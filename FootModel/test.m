...
% Get passive torques
Tau_pass_ankle = f_PassiveMoments(k_pass.ankle,theta.pass.ankle,qs(jointfi.ankle.r),qdots(jointfi.ankle.r));
Tau_pass_subt= f_PassiveMoments(k_pass.subt,theta.pass.subt,qs(jointfi.subt.r),qdots(jointfi.subt.r));
Tau_pass_tmt = f_passiveWLTorques(qs(jointfi.tmt.r),qdots(jointfi.tmt.r),qs(jointfi.mtp.r));

[Tj] = F([qsqdots(:,1);A(:,1);F0(:,1)]);
% Tj = zeros(31,1);
...
f_foot = Function('f_foot',{qs,qdots,A,F0,F_tib_y},{[f1;f2;f3;f4;f5]});
...
Ftib = 0;
opti = casadi.Opti();
qs_opti = opti.variable(nq,1);
opti.subject_to(bounds_qs(:,1) <= qs_opti(:,1));
opti.subject_to(qs_opti(:,1) <= bounds_qs(:,2));
opti.subject_to(qs_opti(jointfi.mtp.r) == qmtp);
qdots_opti = opti.variable(nq,1);
opti.subject_to(qdots_opti == 0);
qddots_opti = opti.variable(nq,1);
opti.subject_to(qddots_opti == 0);

[fsi] = f_foot(qs_opti,qdots_opti,qddots_opti,0,Ftib);
