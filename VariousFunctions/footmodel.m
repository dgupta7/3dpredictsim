function F = footmodel(vars,m0,I0,COM0,ankle0,subtalar0,mtp0)
% variables:
mc  = vars(1);
mf  = vars(2);
Icx = vars(3);
...
sff = abs(mtp_new./mtp); % scale factor forefoot
scm = abs(sff); % scale factor calcn
...
F(9)  = COMf(1) - com_forefoot(1)*sff(1);
F(10) = COMf(2) - com_forefoot(2)*(sff(2));
F(11) = COMf(3) - com_forefoot(3)*sff(3);
...
F(12) = MTJ2COMc(1) - tarsometarsal2com_cm(1)*scm(1);
F(13) = MTJ2COMc(2) - tarsometarsal2com_cm(2)*scm(2);
F(14) = MTJ2COMc(3) - tarsometarsal2com_cm(3)*scm(3);


% F(12) = MTJ2COMc(1)/mtp_new(1) - tarsometarsal2com_cm(1)/mtp(1);
% F(13) = MTJ2COMc(2)/mtp_new(1) - tarsometarsal2com_cm(2)/mtp(1);
% F(14) = MTJ2COMc(3)/mtp_new(1) - tarsometarsal2com_cm(3)/mtp(1);
% 
% F(12) = MTJ2COMc(1)/norm(mtp_new) - tarsometarsal2com_cm(1)/norm(mtp);
% F(13) = MTJ2COMc(2)/norm(mtp_new) - tarsometarsal2com_cm(2)/norm(mtp);
% F(14) = MTJ2COMc(3)/norm(mtp_new) - tarsometarsal2com_cm(3)/norm(mtp);
