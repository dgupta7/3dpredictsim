...
figure(f2)
for j=1:i
    plot(q1*180/pi,M_f_lin(:,j),'--','color',colr(j,:),'DisplayName',[num2str(q2(j)*180/pi) ' full'])
end
...
grid on
for j=1:i
    plot(q1*180/pi,dc1(:,j),'--','color',colr(j,:),'DisplayName',[num2str(q2(j)*180/pi)])
end
