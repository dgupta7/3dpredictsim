...
constexpr int NR = ndof + 5 * 4 + 4 * 3;    // # residual torques + # joint origins
...
int F_generic(const T** arg, T** res) {
	...
	/// ground reaction forces (51-56)
	for (int i = 0; i < nc; ++i) {
		res[0][NU + 20 + i] = value<T>(GRF_r[1][i]);       /// GRF_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][NU + 23 + i] = value<T>(GRF_l[1][i]);  /// GRF_l
	}	
	/// GRF torques (57-62)
	for (int i = 0; i < nc; ++i) {
		res[0][NU + 26 + i] = value<T>(TorqueRight[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][NU + 29 + i] = value<T>(TorqueLeft[i]);
	}
