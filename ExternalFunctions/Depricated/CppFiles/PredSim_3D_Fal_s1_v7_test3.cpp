...
constexpr int NR = ndof*3 + 5 * 4 + 6;   // # residual torques + joint origins + residual torques
...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < ndof; ++i) res[0][NU + 20 + i] = res_os[i];
	for (int i = 0; i < ndof; ++i) res[0][NU*2 + 20 + i] = res_os[i];

	for (int i = 0; i < ndof; ++i) res[0][NU*3 + 20 + i] = value<T>(GRF_r[1][i]);       /// GRF_r
	for (int i = 0; i < ndof; ++i) res[0][NU*3 + 20 + 3 + i] = value<T>(GRF_l[1][i]);   /// GRF_l
