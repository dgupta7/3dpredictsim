...
int F_generic(const T** arg, T** res) {
	...
	Vec3 COMCalcn_r = Vec3(0.066907031412174, 0.033043440839674, -2.064106263057383e-04);
	...
	osim_double_adouble mCalcn = 0.625241534065349;
	Vec3 ITibia = Vec3(0.0356624, 0.00360871, 0.0361578);
	Inertia ITalus(0.000647371, 0.000647371, 0.000647371, 0, 0, 0);
	Vec3 ICalcn = Vec3(6.670471633986739e-04, 8.494673219871866e-04, 0.001011813716159);
	...
	ExoActuation(*model, *tibia_l, *metatarsi_l, *state, ExoTorque_l*0.5, appliedBodyForces);
	ExoActuation(*model, *tibia_r, *metatarsi_r, *state, ExoTorque_r*0.5, appliedBodyForces);
	ExoActuation(*model, *tibia_l, *calcn_l, *state, ExoTorque_l*0.5, appliedBodyForces);
	ExoActuation(*model, *tibia_r, *calcn_r, *state, ExoTorque_r*0.5, appliedBodyForces);
