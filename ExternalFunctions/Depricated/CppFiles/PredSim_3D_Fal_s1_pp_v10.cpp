...
constexpr int NR = ndof + 6 + 5 * 6 + 6 + 6 + 18 + 12 + 6 + 6 + 6;   // # residual torques + joint origins + residual torques 
										// + talus + separate GRFs + separate GRF torques + ankle axis + GRF power + GRF E_pot
...
int F_generic(const T** arg, T** res) {
    ...
    calcn_l = new OpenSim::Body("calcn_l", 0.938544584985273, Vec3(0.082083390736553, 0.009191697741732, 0.004565491687501), Inertia(0.001564300620793, 0.003939605519633, 0.003621431963449, 0, 0, 0));
    calcn_r = new OpenSim::Body("calcn_r", 0.938544584985273, Vec3(0.082083390736553, 0.009191697741732, -0.004565491687501), Inertia(0.001564300620793, 0.003939605519633, 0.003621431963449, 0, 0, 0));
    ...
    ankle_l = new CustomJoint("ankle_l", *tibia_l, Vec3(0, -0.4026, 0), Vec3(0), *talus_l, Vec3(0), Vec3(0), st_ankle_l);
    ankle_r = new CustomJoint("ankle_r", *tibia_r, Vec3(0, -0.4026, 0), Vec3(0), *talus_r, Vec3(0), Vec3(0), st_ankle_r);
    subtalar_l = new CustomJoint("subtalar_l", *talus_l, Vec3(-0.044572, -0.051452, -0.0072383), Vec3(0), *calcn_l, Vec3(0), Vec3(0), st_subtalar_l);
	subtalar_r = new CustomJoint("subtalar_r", *talus_r, Vec3(-0.044572, -0.051452, 0.0072383), Vec3(0), *calcn_r, Vec3(0), Vec3(0), st_subtalar_r);
	...
	Vec3 talus_or_l = talus_l->getPositionInGround(*state);
	...
	SpatialVec GRF_cm_l = GRF_calcn_l + GRF_metatarsi_l;

	SpatialVec GRF_calcn_r = GRF_1_r + GRF_2_r;
	SpatialVec GRF_metatarsi_r = GRF_3_r + GRF_4_r;
	SpatialVec GRF_toes_r = GRF_5_r + GRF_6_r;
	SpatialVec GRF_cm_r = GRF_calcn_r + GRF_metatarsi_r;

	/// Expresss contact wrenches in ground
	Vec3 Torque_ground_calcn_l = GRF_cm_l[0] + cross(calcn_or_l, GRF_cm_l[1]);
	Vec3 Torque_ground_toes_l = GRF_toes_l[0] + cross(toes_or_l, GRF_toes_l[1]);
	Vec3 Torque_ground_calcn_r = GRF_cm_r[0] + cross(calcn_or_r, GRF_cm_r[1]);
	...
	// ankle axis
	Vec3 st_Ankle_l = Vec3(0.10501355, 0.17402245, 0.97912632);
	Vec3 st_Ankle_Gr_l = talus_l->expressVectorInGround(*state,st_Ankle_l) - talus_or_l;
	...
	Vec3 v_HC_3_l = calcn_l->findStationVelocityInGround(*state, locSphere_3_l);
	Vec3 v_HC_4_l = calcn_l->findStationVelocityInGround(*state, locSphere_4_l);
	...
	Vec3 v_HC_3_r = calcn_r->findStationVelocityInGround(*state, locSphere_3_r);
	Vec3 v_HC_4_r = calcn_r->findStationVelocityInGround(*state, locSphere_4_r);
	...
	// contact sphere potential energy
	/// left
	osim_double_adouble E_HC_1_l_y = HC_1_l->computePotentialEnergy(*state);
	osim_double_adouble E_HC_2_l_y = HC_2_l->computePotentialEnergy(*state);
	osim_double_adouble E_HC_3_l_y = HC_3_l->computePotentialEnergy(*state);
	osim_double_adouble E_HC_4_l_y = HC_4_l->computePotentialEnergy(*state);
	osim_double_adouble E_HC_5_l_y = HC_5_l->computePotentialEnergy(*state);
	osim_double_adouble E_HC_6_l_y = HC_6_l->computePotentialEnergy(*state);
    /// per foot segment
	osim_double_adouble E_calcn_l = E_HC_1_l_y + E_HC_2_l_y;
	osim_double_adouble E_metatarsi_l = E_HC_3_l_y + E_HC_4_l_y;
	osim_double_adouble E_toes_l = E_HC_5_l_y + E_HC_6_l_y;

	/// right
	osim_double_adouble E_HC_1_r_y = HC_1_r->computePotentialEnergy(*state);
	osim_double_adouble E_HC_2_r_y = HC_2_r->computePotentialEnergy(*state);
	osim_double_adouble E_HC_3_r_y = HC_3_r->computePotentialEnergy(*state);
	osim_double_adouble E_HC_4_r_y = HC_4_r->computePotentialEnergy(*state);
	osim_double_adouble E_HC_5_r_y = HC_5_r->computePotentialEnergy(*state);
	osim_double_adouble E_HC_6_r_y = HC_6_r->computePotentialEnergy(*state);
    /// per foot segment
	osim_double_adouble E_calcn_r = E_HC_1_r_y + E_HC_2_r_y;
	osim_double_adouble E_metatarsi_r = E_HC_3_r_y + E_HC_4_r_y;
	osim_double_adouble E_toes_r = E_HC_5_r_y + E_HC_6_r_y;
	...
	/// joint origins talus (73-78)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 14] = value<T>(talus_or_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 15] = value<T>(talus_or_l[i]);
	}

	/// separate GRF
	/// GRF_r (79-87)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 16] = value<T>(GRF_calcn_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 17] = value<T>(GRF_metatarsi_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 18] = value<T>(GRF_toes_r[1][i]);
	}
	/// GRF_l (88-96)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 19] = value<T>(GRF_calcn_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 20] = value<T>(GRF_metatarsi_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 21] = value<T>(GRF_toes_l[1][i]);
	}

	/// separate GRF torques r (97-102)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 22] = value<T>(Torque_ground_calcn_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 23] = value<T>(Torque_ground_toes_r[i]);
	}
	/// separate GRF torques l (103-108)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 24] = value<T>(Torque_ground_calcn_l[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 25] = value<T>(Torque_ground_toes_l[i]);
	}

	/// ankle axis in ground ref (109-114)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 26] = value<T>(st_Ankle_Gr_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 27] = value<T>(st_Ankle_Gr_l[i]);
	}

	/// contact sphere deformation power (115-120)
	res[0][0 + ndof + nc * 28] = value<T>(P_calcn_r);
	res[0][1 + ndof + nc * 28] = value<T>(P_metatarsi_r);
	res[0][2 + ndof + nc * 28] = value<T>(P_toes_r);

	res[0][0 + ndof + nc * 29] = value<T>(P_calcn_l);
	res[0][1 + ndof + nc * 29] = value<T>(P_metatarsi_l);
	res[0][2 + ndof + nc * 29] = value<T>(P_toes_l);

    /// contact sphere deformation potential energy (121-126)
	res[0][0 + ndof + nc * 30] = value<T>(E_calcn_r);
	res[0][1 + ndof + nc * 30] = value<T>(E_metatarsi_r);
	res[0][2 + ndof + nc * 30] = value<T>(E_toes_r);

	res[0][0 + ndof + nc * 31] = value<T>(E_calcn_l);
	res[0][1 + ndof + nc * 31] = value<T>(E_metatarsi_l);
	res[0][2 + ndof + nc * 31] = value<T>(E_toes_l);
