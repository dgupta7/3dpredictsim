...
constexpr int NR = ndof + 6 + 6 + 6*3;   // # residual torques + # GRFs + # joint origins + # separate GRFs
...
int F_generic(const T** arg, T** res) {
	...
	calcn_l = new OpenSim::Body("calcn_l", 0.4217, Vec3(0.03628, 0.027101, 0.004057), Inertia(8.5663e-05, 1.7507e-04, 2.0122e-04, 0, 0, 0));
	...
	subtalar_l = new CustomJoint("subtalar_l", *talus_l, Vec3(-0.044572100000000003, -0.05145, -0.0072382799999999997), Vec3(0), *calcn_l, Vec3(0), Vec3(0), st_subtalar_l);
	...
	tmt_l = new PinJoint("tmt_l", *calcn_l, Vec3(0.078023, 0.035452, 0.0068103), Vec3(0), *metatarsi_l, Vec3(0), Vec3(0));
	tmt_r = new PinJoint("tmt_r", *calcn_r, Vec3(0.078023, 0.035452, -0.0068103), Vec3(0), *metatarsi_r, Vec3(0), Vec3(0));
	mtp_l = new PinJoint("mtp_l", *metatarsi_l, Vec3(0.0853859, -0.03728, -0.00779737), Vec3(0), *toes_l, Vec3(0), Vec3(0));
	mtp_r = new PinJoint("mtp_r", *metatarsi_r, Vec3(0.0853859, -0.03728, 0.00779737), Vec3(0), *toes_r, Vec3(0), Vec3(0));
	...
	/// GRF_r (45-53)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 4] = value<T>(GRF_calcn_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 5] = value<T>(GRF_metatarsi_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 6] = value<T>(GRF_toes_r[1][i]);
	}
	/// GRF_l (54-62)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 7] = value<T>(GRF_calcn_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 8] = value<T>(GRF_metatarsi_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 9] = value<T>(GRF_toes_l[1][i]);
	}
