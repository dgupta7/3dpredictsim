...
constexpr int NR = ndof + 5 * 3 + 6 + 3 * 3;   // # joint origins + GRFs + GRFTs (+ COP + lever)
...
int F_generic(const T** arg, T** res) {
	...
	midtarsal_r = new PinJoint("midtarsal_r", *calcn_r, Vec3(0.078024, 0.025452, -0.0068103), Vec3(0), *central_foot_r, Vec3(0), Vec3(0));
	mtp_r = new PinJoint("mtp_r", *central_foot_r, Vec3(0.085386, -0.02728, 0.0077974), Vec3(0), *toes_r, Vec3(0), Vec3(0));
	...
	Vec3 Torque_ground_central_foot_r = GRF_central_foot_r[0] + cross(central_foot_or_r, GRF_central_foot_r[1]);

	Vec3 TorqueRight = Torque_ground_calcn_r + Torque_ground_central_foot_r;


	// Centre of pressure
	Vec3 COP_r;
	COP_r[0] = TorqueRight[2]/GRF_r[1][1];
	COP_r[1] = 0;
	COP_r[2] = -TorqueRight[0]/GRF_r[1][1];

	Vec3 COP_calcn_r;
	COP_calcn_r[0] = Torque_ground_calcn_r[2]/GRF_calcn_r[1][1];
	COP_calcn_r[1] = 0;
	COP_calcn_r[2] = -Torque_ground_calcn_r[0]/GRF_calcn_r[1][1];

	Vec3 COP_central_foot_r;
	COP_central_foot_r[0] = Torque_ground_central_foot_r[2]/GRF_central_foot_r[1][1];
	COP_central_foot_r[1] = 0;
	COP_central_foot_r[2] = -Torque_ground_central_foot_r[0]/GRF_central_foot_r[1][1];


	// Calculate lever arm of GRF to ankle
	SimTK::MobilizedBody Mobil_Talus_r = talus_r.getMobilizedBody();
	Rotation Rot_Talus_r = Mobil_Talus_r.getBodyRotation(*state);
	Vec3 st_Ankle = st_ankle_r[0].getAxes();
	Vec3 st_Ankle_Gr = Rot_Talus_r*st_Ankle;
	Vec3 GRF_distance_vec = COP_r - talus_or_r;
	Vec3 GRF_lever_vec = GRF_distance_vec - dot(GRF_distance_vec,st_Ankle_Gr.normalize());
	osim_double_adouble GRF_lever = GRF_lever_vec.norm();

	Vec3 COP_in_Talus_ref = ~Rot_Talus_r*(-GRF_distance_vec);
	...
	/// total (31-33)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 7] = value<T>(TorqueRight[i]);
	}
	/// separate (34-39)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 8] = value<T>(Torque_ground_calcn_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 9] = value<T>(Torque_ground_central_foot_r[i]);
	}

	/// centre of pressure in ground ref
	/// total (40-42)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 10] = value<T>(COP_r[i]);
	}
	/// separate (43-48)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 11] = value<T>(COP_calcn_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 12] = value<T>(COP_central_foot_r[i]);
	}

	/// centre of pressure in talus ref (49-51)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 13] = value<T>(COP_in_Talus_ref[i]);
	}

	/// ankle axis in ground ref (52-54)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 14] = value<T>(st_Ankle_Gr[i]);
	}

	/// GRF lever arm to ankle (55)
	res[0][ndof + nc * 15] = value<T>(GRF_lever);
