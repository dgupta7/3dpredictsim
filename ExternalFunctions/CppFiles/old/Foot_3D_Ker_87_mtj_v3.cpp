...
constexpr int ndof = 19;           // # degrees of freedom (excluding locked)
constexpr int ndofr = ndof;    // # degrees of freedom (including locked)
constexpr int NX = ndof * 2;       // # states
constexpr int NU = ndof;           // # controls
constexpr int NR = ndof + 5 * 3 + 2;   // # joint origins + GRFs
...
int F_generic(const T** arg, T** res) {
	...
	st_ground_heel[4].setFunction(new Constant());
	st_ground_heel[5].setCoordinateNames(OpenSim::Array<std::string>("heel_tz", 1, 1));
	st_ground_heel[5].setFunction(new Constant());
	...
	st_ground_forefoot[4].setCoordinateNames(OpenSim::Array<std::string>("forefoot_tz", 1, 1));
	st_ground_forefoot[4].setFunction(new LinearFunction());
	st_ground_forefoot[4].setAxis(Vec3(0, 0, 1));
	st_ground_forefoot[5].setCoordinateNames(OpenSim::Array<std::string>("forefoot_ty", 1, 1));
	st_ground_forefoot[5].setFunction(new Constant());
	...
	ground_heel = new CustomJoint("ground_heel", model->getGround(), Vec3(0, 0.010, 0), Vec3(0), *calcn_r, Vec3(0.06, -0.01, 0.012), Vec3(0), st_ground_heel); // 4 dof
	ground_forefoot = new CustomJoint("ground_forefoot", model->getGround(), Vec3(0), Vec3(0), *central_foot_r, Vec3(0.086976, -0.035452,  0.012), Vec3(0), st_ground_forefoot); // 5 dof
	...
	/// GRF heel (1)
	res[0][ndof + nc * 5] = value<T>(GRF_heel[1][1]);

	/// GRF forefoot (1)
	res[0][1 + ndof + nc * 5] = value<T>(GRF_forefoot[1][1]);
