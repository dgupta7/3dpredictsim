...
constexpr int NR = ndof + 6 + 6 + 18 + 6 + 6 + 18 + 6 + 6;   // # residual torques + # GRFs + # joint origins 
		// + # separate GRFs + GRF torques + talus origin + separate GRF torques + ankle axis + GRF power
...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::Body* metatarsi_r; //includes midfoot and forefoot
	OpenSim::Body* metatarsi_l; //includes midfoot and forefoot
	OpenSim::Body* toes_r;
	OpenSim::Body* toes_l;
	OpenSim::Body* torso;
    ...
    calcn_l = new OpenSim::Body("calcn_l", 0.40857, Vec3(0.036283, 0.017101, 0.004057), Inertia(8.2986e-05, 0.0001696, 0.00019494, 0, 0, 0));
	calcn_r = new OpenSim::Body("calcn_r", 0.40857, Vec3(0.036283, 0.017101, -0.004057), Inertia(8.2986e-05, 0.0001696, 0.00019494, 0, 0, 0));
	metatarsi_l = new OpenSim::Body("metatarsi_l", 0.52998, Vec3(0.039368, -0.014649, -0.0018528), Inertia(0.0013195, 0.0021355, 0.0018662, 0, 0, 0));
	metatarsi_r = new OpenSim::Body("metatarsi_r", 0.52998, Vec3(0.039368, -0.014649, 0.0018528), Inertia(0.0013195, 0.0021355, 0.0018662, 0, 0, 0));
	...
	midtarsal_l = new PinJoint("midtarsal_l", *calcn_l, Vec3(0.078024, 0.025452, 0.0068103), Vec3(0), *metatarsi_l, Vec3(0), Vec3(0));
	midtarsal_r = new PinJoint("midtarsal_r", *calcn_r, Vec3(0.078024, 0.025452, -0.0068103), Vec3(0), *metatarsi_r, Vec3(0), Vec3(0));
	mtp_l = new PinJoint("mtp_l", *metatarsi_l, Vec3(0.085386, -0.02728, -0.0077974), Vec3(0), *toes_l, Vec3(0), Vec3(0));
	mtp_r = new PinJoint("mtp_r", *metatarsi_r, Vec3(0.085386, -0.02728, 0.0077974), Vec3(0), *toes_r, Vec3(0), Vec3(0));
	...
	model->addBody(metatarsi_l);	model->addJoint(midtarsal_l);
	model->addBody(metatarsi_r);	model->addJoint(midtarsal_r);
	...
	osim_double_adouble stiffness = 10000000;
	...
	Vec3 locSphere_4_r(0.086976, -0.035452, -0.0031897);
	...
	osim_double_adouble P_metatarsi_l = P_HC_3_l_y + P_HC_4_l_y + P_HC_6_l_y;
	osim_double_adouble P_toes_l = P_HC_5_l_y; // sphere 6 is not located distal to the mtp joint, so it is considered as forefoot deformation power
	...
	osim_double_adouble P_metatarsi_r = P_HC_3_r_y + P_HC_4_r_y + P_HC_6_r_y;
	osim_double_adouble P_toes_r = P_HC_5_r_y; // sphere 6 is not located distal to the mtp joint, so it is considered as forefoot deformation power
	...
	/// GRF_r (45-53)
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 4] = value<T>(GRF_calcn_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 5] = value<T>(GRF_metatarsi_r[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 6] = value<T>(GRF_toes_r[1][i]);
	}
	/// GRF_l (54-62)
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 7] = value<T>(GRF_calcn_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 8] = value<T>(GRF_metatarsi_l[1][i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + NU + nc * 9] = value<T>(GRF_toes_l[1][i]);
	}

	/// GRF torques (63-68)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 10] = value<T>(TorqueRight[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 11] = value<T>(TorqueLeft[i]);
	}

	/// joint origins talus (69-74)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 12] = value<T>(talus_or_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 13] = value<T>(talus_or_l[i]);
	}

	/// separate GRF torques r (75-83)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 14] = value<T>(Torque_ground_calcn_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 15] = value<T>(Torque_ground_metatarsi_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 16] = value<T>(Torque_ground_toes_r[i]);
	}
	/// separate GRF torques l (84-92)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 17] = value<T>(Torque_ground_calcn_l[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 18] = value<T>(Torque_ground_metatarsi_l[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 19] = value<T>(Torque_ground_toes_l[i]);
	}

	// ankle axis in ground ref (93-98)
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 20] = value<T>(st_Ankle_Gr_r[i]);
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 21] = value<T>(st_Ankle_Gr_l[i]);
	}

	/// contact sphere deformation power (99-104)
