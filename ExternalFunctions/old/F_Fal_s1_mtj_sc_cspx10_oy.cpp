...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::Body* pelvis;
	pelvis = new OpenSim::Body("pelvis", 9.55002108265680327293, Vec3(-0.06827780017111793887, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.08011447798433828094, 0.08011447798433828094, 0.04378934326383828596, 0., 0., 0.));
	model->addBody(pelvis);

	OpenSim::Body* femur_r;
	femur_r = new OpenSim::Body("femur_r", 7.54254615761433289123, Vec3(0.00000000000000000000, -0.17046665777369882089, 0.00000000000000000000), Inertia(0.10917703160243863447, 0.02861922187636740597, 0.11512917746276575548, 0., 0., 0.));
	...
	femur_l = new OpenSim::Body("femur_l", 7.54254615761433289123, Vec3(0.00000000000000000000, -0.17046665777369882089, 0.00000000000000000000), Inertia(0.10917703160243863447, 0.02861922187636740597, 0.11512917746276575548, 0., 0., 0.));
	model->addBody(femur_l);

	OpenSim::Body* tibia_l;
	tibia_l = new OpenSim::Body("tibia_l", 3.00564638032839148707, Vec3(0.00000000000000000000, -0.17525042630686288314, 0.00000000000000000000), Inertia(0.03600213210381681544, 0.00364487209617144490, 0.03600213210381681544, 0., 0., 0.));
	model->addBody(tibia_l);

	OpenSim::Body* talus_l;
	talus_l = new OpenSim::Body("talus_l", 0.11461076105861879582, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.00156142780048287269, 0.00145393819892659882, 0.00156142780048287269, 0., 0., 0.));
	model->addBody(talus_l);

	OpenSim::Body* calcn_l;
	calcn_l = new OpenSim::Body("calcn_l", 0.61234489257640645210, Vec3(0.03570388663594165068, 0.00751164358725005948, 0.00442138678000001100), Inertia(0.00082129480699425330, 0.00000000000000000000, 0.00082129480699425330, 0., 0., 0.));
	model->addBody(calcn_l);

	OpenSim::Body* midfoot_l;
	midfoot_l = new OpenSim::Body("midfoot_l", 0.29545720755743304364, Vec3(0.01395438483210798865, -0.00639769662427499934, 0.00260126982999999996), Inertia(0.00044030645909543523, 0.00000000000000000000, 0.00044030645909543523, 0., 0., 0.));
	model->addBody(midfoot_l);

	OpenSim::Body* forefoot_l;
	forefoot_l = new OpenSim::Body("forefoot_l", 0.49884953688625716106, Vec3(0.03109280751133555037, -0.00818154511514999905, 0.00047461483600000001), Inertia(0.00109001190950101584, 0.00105379340332094253, 0.00105379340332094253, 0., 0., 0.));
	model->addBody(forefoot_l);

	OpenSim::Body* toes_l;
	toes_l = new OpenSim::Body("toes_l", 0.27422978466560654631, Vec3(0.03543281535775673341, 0.00708790499999999938, 0.02007250000000000353), Inertia(0.00018058881524868320, 0.00024452548712495067, 0.00024452548712495067, 0., 0., 0.));
	model->addBody(toes_l);

	OpenSim::Body* torso;
	torso = new OpenSim::Body("torso", 21.75380789471011055980, Vec3(-0.02676026676359908804, 0.30650513962530268053, 0.00000000000000000000), Inertia(1.05966189872782967640, 0.48746393661965370381, 1.05966189872782967640, 0., 0., 0.));
	model->addBody(torso);

	OpenSim::Body* humerus_r;
	humerus_r = new OpenSim::Body("humerus_r", 1.64816318676232964613, Vec3(0.00000000000000000000, -0.16903343413648269644, 0.00000000000000000000), Inertia(0.01022810153093832615, 0.00352837823614572622, 0.01148071433353022378, 0., 0., 0.));
	model->addBody(humerus_r);

	OpenSim::Body* ulna_r;
	ulna_r = new OpenSim::Body("ulna_r", 0.49262442113560389600, Vec3(0.00000000000000000000, -0.11693148015018021979, 0.00000000000000000000), Inertia(0.00226080613707762678, 0.00047170094284739144, 0.00245238694072600098, 0., 0., 0.));
	model->addBody(ulna_r);

	OpenSim::Body* radius_r;
	radius_r = new OpenSim::Body("radius_r", 0.49262442113560389600, Vec3(0.00000000000000000000, -0.11693148015018021979, 0.00000000000000000000), Inertia(0.00226080613707762678, 0.00047170094284739144, 0.00245238694072600098, 0., 0., 0.));
	model->addBody(radius_r);

	OpenSim::Body* hand_r;
	hand_r = new OpenSim::Body("hand_r", 0.37098876159594867952, Vec3(0.00000000000000000000, -0.06606470973512983125, 0.00000000000000000000), Inertia(0.00068083695957908274, 0.00041750876332932545, 0.00102278197963673865, 0., 0., 0.));
	model->addBody(hand_r);

	OpenSim::Body* humerus_l;
	humerus_l = new OpenSim::Body("humerus_l", 1.64816318676232964613, Vec3(0.00000000000000000000, -0.16903343413648269644, 0.00000000000000000000), Inertia(0.01022810153093832615, 0.00352837823614572622, 0.01148071433353022378, 0., 0., 0.));
	model->addBody(humerus_l);

	OpenSim::Body* ulna_l;
	ulna_l = new OpenSim::Body("ulna_l", 0.49262442113560389600, Vec3(0.00000000000000000000, -0.11693148015018021979, 0.00000000000000000000), Inertia(0.00226080613707762678, 0.00047170094284739144, 0.00245238694072600098, 0., 0., 0.));
	model->addBody(ulna_l);

	OpenSim::Body* radius_l;
	radius_l = new OpenSim::Body("radius_l", 0.49262442113560389600, Vec3(0.00000000000000000000, -0.11693148015018021979, 0.00000000000000000000), Inertia(0.00226080613707762678, 0.00047170094284739144, 0.00245238694072600098, 0., 0., 0.));
	model->addBody(radius_l);

	OpenSim::Body* hand_l;
	hand_l = new OpenSim::Body("hand_l", 0.37098876159594867952, Vec3(0.00000000000000000000, -0.06606470973512983125, 0.00000000000000000000), Inertia(0.00068083695957908274, 0.00041750876332932545, 0.00102278197963673865, 0., 0., 0.));
	...
	st_ankle_l[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.93891806785062925655));
	st_ankle_l[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_ankle_l[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.93867394915298807412));
	st_ankle_l[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_ankle_l[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.93891806785062925655));
	...
	ankle_l = new OpenSim::CustomJoint("ankle_l", *tibia_l, Vec3(0.00000000000000000000, -0.40362979813578486743, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *talus_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_ankle_l);
	...
	st_subtalar_l[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.90118143106433312717));
	st_subtalar_l[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_subtalar_l[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.42939417499999987804));
	st_subtalar_l[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_subtalar_l[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.99789000000000005475));
	...
	subtalar_l = new OpenSim::CustomJoint("subtalar_l", *talus_l, Vec3(-0.04395061839300753309, -0.05996308564124999685, -0.00790328879999999991), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *calcn_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_subtalar_l);

	OpenSim::PinJoint* midtarsal_l;
	midtarsal_l = new OpenSim::PinJoint("midtarsal_l", *calcn_l, Vec3(0.07723073660730933598, 0.02919002465610008590, 0.00743489299800000319), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *midfoot_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));

	OpenSim::WeldJoint* tarsometatarsal_l;
	tarsometatarsal_l = new OpenSim::WeldJoint("tarsometatarsal_l", *midfoot_l, Vec3(0.02300583064432199301, -0.01558453111874999811, -0.00524434781000000013), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *forefoot_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));

	OpenSim::PinJoint* mtp_l;
	mtp_l = new OpenSim::PinJoint("mtp_l", *forefoot_l, Vec3(0.06194209122232211678, -0.01664216467649999587, -0.00328990568999999993), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *toes_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	...
	Vec3 SmoothSphereHalfSpaceForce_s5_r_location(0.05315399999999999986, -0.00153854124456095573, -0.00341729999999999985);
	...
	SmoothSphereHalfSpaceForce_s5_r->set_stiffness(10000000.00000000000000000000);
	...
	Vec3 SmoothSphereHalfSpaceForce_s6_r_location(0.00000173810000000000, -0.00153854124456095573, 0.02229400000000000132);
	...
	SmoothSphereHalfSpaceForce_s6_r->set_stiffness(10000000.00000000000000000000);
	...
	Vec3 SmoothSphereHalfSpaceForce_s1_l_location(-0.00042151999999999999, 0.00692291751087808875, 0.00499720000000000025);
	...
	SmoothSphereHalfSpaceForce_s1_l->set_stiffness(10000000.00000000000000000000);
	...
	Vec3 SmoothSphereHalfSpaceForce_s2_l_location(0.05999999999999999778, 0.00692291751087808875, -0.02000100000000000142);
	...
	SmoothSphereHalfSpaceForce_s2_l->set_stiffness(10000000.00000000000000000000);
	...
	SmoothSphereHalfSpaceForce_s3_l = new SmoothSphereHalfSpaceForce("SmoothSphereHalfSpaceForce_s3_l", *midfoot_l, model->getGround());
	Vec3 SmoothSphereHalfSpaceForce_s3_l_location(0.08697599999999999776, -0.02699054124456095466, -0.02799300000000000052);
	...
	SmoothSphereHalfSpaceForce_s3_l->set_stiffness(10000000.00000000000000000000);
	...
	SmoothSphereHalfSpaceForce_s3_l->connectSocket_sphere_frame(*midfoot_l);
	...
	SmoothSphereHalfSpaceForce_s4_l = new SmoothSphereHalfSpaceForce("SmoothSphereHalfSpaceForce_s4_l", *midfoot_l, model->getGround());
	Vec3 SmoothSphereHalfSpaceForce_s4_l_location(0.08697599999999999776, -0.02699054124456095466, 0.00318970000000000017);
	...
	SmoothSphereHalfSpaceForce_s4_l->set_stiffness(10000000.00000000000000000000);
	...
	SmoothSphereHalfSpaceForce_s4_l->connectSocket_sphere_frame(*midfoot_l);
	...
	Vec3 SmoothSphereHalfSpaceForce_s5_l_location(0.05315399999999999986, -0.00153854124456095573, 0.00341729999999999985);
	...
	SmoothSphereHalfSpaceForce_s5_l->set_stiffness(10000000.00000000000000000000);
	...
	Vec3 SmoothSphereHalfSpaceForce_s6_l_location(0.00000173810000000000, -0.00153854124456095573, -0.02229400000000000132);
	...
	SmoothSphereHalfSpaceForce_s6_l->set_stiffness(10000000.00000000000000000000);
	...
	int c_idx_8 = model->getBodySet().get("midfoot_l").getMobilizedBodyIndex();
	...
	int c_idx_9 = model->getBodySet().get("midfoot_l").getMobilizedBodyIndex();
	...
	SimTK::Transform TR_GB_midfoot_r = midfoot_r->getMobilizedBody().getBodyTransform(*state);
	Vec3 SmoothSphereHalfSpaceForce_s3_r_location_G = midfoot_r->findStationLocationInGround(*state, SmoothSphereHalfSpaceForce_s3_r_location);
	...
	Vec3 SmoothSphereHalfSpaceForce_s3_r_locationCP_B = model->getGround().findStationLocationInAnotherFrame(*state, locationCP_G_adj_2, *midfoot_r);
	Vec3 GRM_2 = (TR_GB_midfoot_r*SmoothSphereHalfSpaceForce_s3_r_locationCP_B) % GRF_2[1];
	GRM_r += GRM_2;

	Vec3 SmoothSphereHalfSpaceForce_s4_r_location_G = midfoot_r->findStationLocationInGround(*state, SmoothSphereHalfSpaceForce_s4_r_location);
	...
	Vec3 SmoothSphereHalfSpaceForce_s4_r_locationCP_B = model->getGround().findStationLocationInAnotherFrame(*state, locationCP_G_adj_3, *midfoot_r);
	Vec3 GRM_3 = (TR_GB_midfoot_r*SmoothSphereHalfSpaceForce_s4_r_locationCP_B) % GRF_3[1];
	GRM_r += GRM_3;
	...
	SimTK::Transform TR_GB_midfoot_l = midfoot_l->getMobilizedBody().getBodyTransform(*state);
	Vec3 SmoothSphereHalfSpaceForce_s3_l_location_G = midfoot_l->findStationLocationInGround(*state, SmoothSphereHalfSpaceForce_s3_l_location);
	...
	Vec3 SmoothSphereHalfSpaceForce_s3_l_locationCP_B = model->getGround().findStationLocationInAnotherFrame(*state, locationCP_G_adj_8, *midfoot_l);
	Vec3 GRM_8 = (TR_GB_midfoot_l*SmoothSphereHalfSpaceForce_s3_l_locationCP_B) % GRF_8[1];
	GRM_l += GRM_8;

	Vec3 SmoothSphereHalfSpaceForce_s4_l_location_G = midfoot_l->findStationLocationInGround(*state, SmoothSphereHalfSpaceForce_s4_l_location);
	...
	Vec3 SmoothSphereHalfSpaceForce_s4_l_locationCP_B = model->getGround().findStationLocationInAnotherFrame(*state, locationCP_G_adj_9, *midfoot_l);
	Vec3 GRM_9 = (TR_GB_midfoot_l*SmoothSphereHalfSpaceForce_s4_l_locationCP_B) % GRF_9[1];
	GRM_l += GRM_9;
