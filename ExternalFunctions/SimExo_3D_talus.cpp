...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc*2] = value<T>(calcn_or_r[i]);      /// calcn_or_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc*3] = value<T>(calcn_or_l[i]); /// calcn_or_l
	}

	/// Joint origins - femur 43-48
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 4] = value<T>(femur_or_r[i]);      /// calcn_or_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 5] = value<T>(femur_or_l[i]); /// calcn_or_l
	}

	/// Joint origins - hand 49-54
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 6] = value<T>(hand_or_r[i]);      /// calcn_or_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 7] = value<T>(hand_or_l[i]); /// calcn_or_l
	}

	/// Joint origins tibia 55-60
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 8] = value<T>(tibia_or_r[i]);      /// calcn_or_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 9] = value<T>(tibia_or_l[i]); /// calcn_or_l
	}

	/// Joint origins toes 61-66
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 10] = value<T>(toes_or_r[i]);      /// calcn_or_r
	}
	for (int i = 0; i < nc; ++i) {
		res[0][i + ndof + nc * 11] = value<T>(toes_or_l[i]); /// calcn_or_l
	}
